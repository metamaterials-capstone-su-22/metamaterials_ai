...
I_INC_CKPT_PATH = '../local_work/saved_best/I-1-res-ann-inconel.ckpt'
# ../data/models/model.ckpt"
I_SS_CKPT_PATH = '../local_work/saved_best/I-1-res-ann-stainless.ckpt'
# ../data/models/model.ckpt"
D_INC_CKPT_PATH = '../local_work/saved_best/D-1-res-ann-inconel-2022-07-15_23-44.ckpt'
# ../data/models/model.ckpt"
D_SS_CKPT_PATH = '../local_work/saved_best/D-1-res-ann-stainless.ckpt'
INC_DATA_PATH = '../local_data/inconel-onehot.pt'
SS_DATA_PATH = '../local_data/stainless-onehot.pt'
...
class Predictor:
    def __init__(
        ...
        self.ss_max_speed, self.ss_max_spacing = steel_params.max(
            0)[0][0].item(), steel_params.max(0)[0][1].item()
        self.ss_min_speed, self.ss_min_spacing = steel_params.min(
            0)[0][0].item(), steel_params.min(0)[0][1].item()
        self.inc_max_speed, self.inc_max_spacing = inc_params.max(
            0)[0][0].item(), inc_params.max(0)[0][1].item()
        self.inc_min_speed, self.inc_min_spacing = inc_params.min(
            0)[0][0].item(), inc_params.min(0)[0][1].item()
    ...
    def clean_and_send_to_lab(self, y_hat):
        ...
        unique_data = sorted([list(x) for x in set(
            tuple(x) for x in y_final.tolist())], key=lambda x: (x[2], x[0], x[1]))
    ...
    def run_model(self, input, substrate, direction):
        ...
        if substrate == "inconel" and direction == "inverse":
            ...
            raise Exception(
                f'Model path not correctly specified or does not exist at {path}!')
    ...
    def find_in_data(self, y_hat, substrate):
        ...
        if substrate == 'steel':
            wave, emiss, laser_params, uids = self.get_data(
                SS_DATA_PATH, "full")
            wave_test, emiss_test, laser_params_test, uids_test = self.get_data(
                SS_DATA_PATH, "test")
        else:
            wave, emiss, laser_params, uids = self.get_data(
                INC_DATA_PATH, "full")
        ...
        if len(test_in_data) != 0:
            test_in_data["laser_params"] = torch.stack(
                test_in_data["laser_params"])
            test_in_data["emissivity"] = torch.stack(
                test_in_data["emissivity"])
            test_in_data["uids"] = torch.stack(test_in_data["uids"])
        pred_in_data["laser_params"] = torch.stack(
            pred_in_data["laser_params"])
    ...
    def run_direct(self, y_hat_ss, y_hat_inc, laser_constraint=False):
        ...
        if not Path.is_file(direct_inc_filepath):
            raise Exception(
                f'Model file does not exist at {direct_inc_filepath}!')
        ...
        if not Path.is_file(direct_ss_filepath):
            raise Exception(
                f'Model file does not exist at {direct_ss_filepath}!')
    ...
    def SAD(self, preds, test, substrate):
        ...
        pred_params = self.denormalize_and_decode(
            preds["laser_params"], substrate=substrate, change_dimension=True, normalize="normalize", round_laser_params=False)
        original_params = self.denormalize_and_decode(
            test["laser_params"], substrate=substrate, change_dimension=True, normalize="normalize", round_laser_params=False)

        # non normalized laser parameters for output
        pred = self.denormalize_and_decode(
            preds["laser_params"], substrate=substrate, change_dimension=True, normalize="None", round_laser_params=True)
        original = self.denormalize_and_decode(
            test["laser_params"], substrate=substrate, change_dimension=True, normalize="None", round_laser_params=True)
        # calculate SAD
        difference = torch.abs(torch.sub(pred_params, original_params))
        sad_values = torch.sum(difference, dim=1)
        rmse_val = torch.tensor([rmse(preds["emissivity"][i], test["emissivity"][i])
                                for i in range(len(preds["emissivity"]))])
