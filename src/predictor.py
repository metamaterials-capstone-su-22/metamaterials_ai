...
class Predictor:
    def __init__(
        ...
        self.ss_max_speed, self.ss_max_spacing = (
            steel_params.max(0)[0][0].item(),
            steel_params.max(0)[0][1].item(),
        )
        self.ss_min_speed, self.ss_min_spacing = (
            steel_params.min(0)[0][0].item(),
            steel_params.min(0)[0][1].item(),
        )
        self.inc_max_speed, self.inc_max_spacing = (
            inc_params.max(0)[0][0].item(),
            inc_params.max(0)[0][1].item(),
        )
        self.inc_min_speed, self.inc_min_spacing = (
            inc_params.min(0)[0][0].item(),
            inc_params.min(0)[0][1].item(),
        )

    def denormalize_and_decode(
        self,
        y_hat,
        substrate,
        change_dimension=False,
        normalize="denormalize",
        round_laser_params=True,
    ):
    ...
    def clean_and_send_to_lab(self, y_hat):
        ...
        unique_data = sorted(
            [list(x) for x in set(tuple(x) for x in y_final.tolist())],
    ...
    def get_data(self, filepath, type):
        stain_data = torch.load(
            Path(filepath)
        )  # ["laser_params"] #stainless-steel-revised-shuffled inconel-revised-raw-shuffled
        ...
        if type == "test":
            wave_test = wave_data[round(len(wave_data) * 0.9) :]
            emiss_test = emiss_data[round(len(wave_data) * 0.9) :]
            laser_params = laser_params[round(len(wave_data) * 0.9) :]
            uids = uids[round(len(wave_data) * 0.9) :]
        elif type == "trainval":
            wave_test = wave_data[: round(len(wave_data) * 0.9)]
            emiss_test = emiss_data[: round(len(wave_data) * 0.9)]
            laser_params = laser_params[: round(len(wave_data) * 0.9)]
    ...
    def best_predictor(
        ...
        include_inconel,
        include_stainless,
        direct_ss_emiss_y_hat,
        direct_inc_emiss_y_hat,
        desired_emiss,
    ):
        ...
        if (
            include_inconel
            and include_stainless
            and rmse(direct_inc_emiss_y_hat, desired_emiss)
            < rmse(direct_ss_emiss_y_hat, desired_emiss)
        ) or (include_inconel and not include_stainless):
            best_substrate = substrates[0]
            best_params = self.denormalize_decode_result(
                direct_inc_emiss_y_hat,
                self.inc_max_speed,
                self.inc_max_spacing,
                self.inc_min_speed,
                self.inc_min_spacing,
            )
        ...
        best_params = [
            torch.round(best_params[0], decimals=1).item(),
            torch.round(best_params[1], decimals=1).item(),
            torch.round(best_params[2], decimals=1).item(),
        ]
    ...
    def SAD(self, preds, test, substrate):
        ...
        rmse_val = torch.tensor(
            [
                rmse(preds["emissivity"][i], test["emissivity"][i])
        ...
        return (
            pred,
            original,
            sad_values.reshape(sad_values.shape[0], 1),
            rmse_val.reshape(rmse_val.shape[0], 1),
            preds["uids"],
            test["uids"],
        )
    ...
    def rmse_metrics(y_hat, y_pred, decimals=5):
        ...
        return np.round(np.mean(rmse_scores), decimals=decimals), np.round(
            np.std(rmse_scores), decimals=decimals
        )
