        ...
        if stride != 1 or in_channels != out_channels:
            if option == "A":
                ...
                self.shortcut = LambdaLayer(
                    lambda x: F.pad(
                        x[:, :, ::2, ::2], (0, 0, 0, 0, pad_to_add, pad_to_add, 0, 0)
                    )
                )
            if option == "B":
                self.shortcut = nn.Sequential(
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.block1 = self.__build_layer(
            block_type, out_channels, num_blocks[0], starting_stride=1
        )

        self.block2 = self.__build_layer(
            block_type, out_channels, num_blocks[1], starting_stride=1
        )

        self.block3 = self.__build_layer(
            block_type, out_channels, num_blocks[2], starting_stride=1
        )
